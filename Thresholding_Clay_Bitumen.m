...
cd VBAMday21 %Folder where tif images are located
Files=dir('*ch01.tif'); %Read all of the images that end with ch01.tif (Usually Clay channel) 
Files2=dir('*ch00.tif');%Read all of the images that end with ch00.tif (Usually Bitumen channel)
...
for i=1:ak(end) %to evaluate each layer (each image)
    ...
    %Threshholding
    bw = imbinarize(Iobrcbr,'adaptive','ForegroundPolarity','bright','Sensitivity',0.8); % As you reduce the '0.8' value, only brighter values are considered.
...
%Normal Thresholding
    imch2=adapthisteq(imch2);
    imch2=imfilter(imch2,ones(3)/3^2);
    imch2_i(:,:,i)=imch2;
    BWfinalgreen=im2bw(imch2,0.2); % as you increase the second value [0 1], only the brightest pixels are considered 
...
% cd 'D:\Edna\Aleks-December19th' %Go back to the previous folder where the function is located
% 
% [I3,Z]=Bitumen_squish_Fr1(bw_bitumen);

 %% This section removes the squished Bitumen from the Clay images
 
for i=1:ak(end)
   Clay=imread(strcat('Clay',num2str(i),'.tiff')); 
   Bitumen=imread(strcat('Bitumen',num2str(i),'.tiff'));
   ...
   imwrite(imcomplement(Clay_bit_plus(:,:,i)),strcat('Water',num2str(i),'.tiff'));
end
...
% 
% Df_3d=Box_Counting_3D(resxy,resz,ak(end),Clay_bit);

% Outlined Original image/Clay
BWoutline=bwperim(bw_i(:,:,1));% last number indicates layer that you want to see
Segout=imread(strcat(Files(ak(1)).name));
